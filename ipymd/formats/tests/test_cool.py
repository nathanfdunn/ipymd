...
def _test_markdown_reader(basename, ignore_notebook_meta=False):
    ...
    converted, expected = _test_reader(basename, 'coolfmt',
...
def _test_markdown_writer(basename):
    ...
    """
    converted, expected = _test_writer(basename, 'coolfmt')
...
def _test_markdown_markdown(basename):
    ...
    contents = _read_test_file(basename, 'coolfmt')
    cells = convert(contents, from_='coolfmt')
    converted = convert(cells, to='coolfmt')
...
def test_decorator():
    ...
    markdown_bis = convert(cells, to='coolfmt')
